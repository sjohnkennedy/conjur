module Util
  ...
  class ConcurrencyLimitedCache
    ...
    def call(**args)
      @concurrency_mutex.synchronize do
        if @concurrent_requests >= @max_concurrent_requests
          @logger.info(Log::ConcurrencyLimitedCacheReached.new(@max_concurrent_requests))
          raise Err::ConcurrencyLimitReachedBeforeCacheInitialization unless @cache.key?(args)
          return @cache[args]
        end

        @concurrent_requests += 1
        @logger.info(Log::ConcurrencyLimitedCacheConcurrentRequestsUpdated.new(@concurrent_requests))
      end
    ...
    def recalculate(args)
      ...
      @logger.info(Log::ConcurrencyLimitedCacheUpdated.new)
