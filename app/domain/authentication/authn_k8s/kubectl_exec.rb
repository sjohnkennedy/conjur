    ...
    KubectlExec ||= CommandClass.new(
      ...
      def on_open(ws_client, body, stdin)
        ...
        if hs_error
          ...
          @logger.info(Log::PodChannelOpen.new(@pod_name))
      ...
      def on_message(msg, ws_client)
        ...
        if msg_type == :binary
          @logger.info(Log::PodChannelData.new(@pod_name, wsmsg.channel_name, msg_data))
          @message_log.save_message(wsmsg)
        elsif msg_type == :close
          @logger.info(Log::PodMessageData.new(@pod_name, "close", msg_data))
          ws_client.close
        end
      end
      ...
      def on_close
        @channel_closed = true
        @logger.info(Log::PodChannelClosed.new(@pod_name))
      end
      ...
      def on_error(err)
        ...
        @logger.info(Log::PodError.new(@pod_name, error_info))
